...
export async function handleChatTurn({ message, context }) {
  ...
  const applianceType = inferApplianceType(context?.applianceType, userMessage);
  const modelNumber = context?.modelNumber || extractModelNumber(userMessage);
  const psNumber = context?.selectedPsNumber || extractPsNumber(userMessage);
  const toolCalls = [];
  ...
  if (intent === "COMPATIBILITY_CHECK") {
    ...
    const part = await callTool(toolCalls, "get_part_details", { psNumber });
    const compatibility = {
      modelNumber,
      psNumber,
