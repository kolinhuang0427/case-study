...
function executeToolHandler(handler, input) {
  // Defers invocation into a promise chain so both sync throws and rejected
  // async handlers follow the same error path.
...
export async function runTool(toolName, input, options = {}) {
  ...
  try {
    const data = await Promise.race([
      executeToolHandler(contract.handler, input),
